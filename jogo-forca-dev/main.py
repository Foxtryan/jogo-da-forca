...
# version: 1.7.1 - dev
# situacao: APK e EXE ok
# adicoes nao testadas: botao 'esc' e 'voltar' fisicos
...
# Caso o banco nao exista ele eh criado aqui
try:
	teste = store['jogador']
except:

	store.put("jogador", pontos=0, splash="base")
	store.put("loja", base=["image/base/7.jpg", "0", "base"], um=["image/um/7.jpg", "15", "um"])
...
class Jogo(Screen):
	...
	def __init__(self, *args, **kwargs):
		...
		# Para botao 'voltar' do android funcionar
	...
	def saveData(self, condicao):
		...
		#try:
		if condicao == 'vitoria':
			self.data['pontos'] = str(int(self.data['pontos'])+1)
...
class Loja(Screen):
	...
	def saveData(self, condicao, variavel):
		...
		if int(self.pontos) >= int(valor):
			...
			store['jogador']['pontos'] = str(self.pontos)
			self.loadData()
	...
	def loadData(self):
		...
		self.data = store['jogador']
		self.pontos = str(store['jogador']['pontos'])

	def voltar(self):
		self.manager.current = 'inicio'
		self.manager.get_screen('jogo').path_splash = self.data['splash']

	# botao android 'voltar' ou pc 'esc'
	def Android_back_click(self,window,key,*largs):
		if key == 27:
			self.manager.current='inicio'
			return True
